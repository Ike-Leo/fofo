...
export default function ProductDetailPage({
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-10">
            {/* Header Actions */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-card p-4 rounded-xl border border-border shadow-sm gap-4">
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    {product.status !== 'active' && (
                        <button
                            ...
                            className="flex-1 md:flex-none inline-flex justify-center items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-emerald-500 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-lg transition-colors border border-emerald-500/20"
                        >
                            <PlayCircle size={16} />
                            Publish
                        </button>
                    ...
                    {product.status !== 'draft' && (
                        <button
                            ...
                            className="flex-1 md:flex-none inline-flex justify-center items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-amber-500 bg-amber-500/10 hover:bg-amber-500/20 rounded-lg transition-colors border border-amber-500/20"
                        >
                            <EyeOff size={16} />
                            Unpublish
                        </button>
                    ...
                    {product.status !== 'archived' && (
                        <button
                            ...
                            className="flex-1 md:flex-none inline-flex justify-center items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-slate-400 bg-slate-500/10 hover:bg-slate-500/20 rounded-lg transition-colors border border-slate-500/20"
                        >
                            <Archive size={16} />
                            Archive
                        </button>
